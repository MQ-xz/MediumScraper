		...
		try:		
			...
			for user,post in zip(data['references']['User'],data['references']['Post']):
				...
				try:
					...
					dt = (f'''\033[32;1m{title} by {username}
		\033[0m{subtitle}
		\033[34;4m{url}\033[0m
				''')
					if dt not in self.mList:
						print(dt)
						self.mList.append(dt)
				except:
					url = f'https://medium.com/{username}/{postdata["uniqueSlug"]}'
					dt = (f'''\033[32;1m{title} by {username}
		\033[34;4m{url}\033[0m
	...
	def main(self):
		try:
			if self.args.year is not None:
				...
				for y in self.years:
					if str(y) == "2014" or str(y) == "2015":
						for m in self.months:
							self.scraper(self.args.tag, y, m, "01")
					else:
						for m in self.months:
							for d in range(1, self.get_days(y, m) +1):
								year = str(self.args.year)
								d = (f"{d:02d}")
								print(d)
